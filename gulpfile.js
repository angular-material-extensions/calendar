...
const config = {
  libraryName: '@angular-material-extensions/calendar',
  unscopedLibraryName: 'calendar',
  allSrc: 'src/**/*',
  allTs: 'src/**/!(*.spec).ts',
...
gulp.task('build:demo', () => {
  return execDemoCmd(`build --preserve-symlinks --prod --base-href /calendar/ --deploy-url /calendar/`, {cwd: `${config.demoDir}`});
});
...
gulp.task('build:demo-ssr', () => {
  return execDemoCmd(`build --preserve-symlinks --prod`, {cwd: `${config.demoDir}`})
    .then(() => execDemoCmd(`run @angular-material-extensions/calendar-demo:server`, {cwd: `${config.demoDir}`}))
